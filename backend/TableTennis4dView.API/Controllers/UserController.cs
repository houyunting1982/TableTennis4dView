...
using TableTennis4dView.Application.Commands.User.Update;
using TableTennis4dView.Application.DTOs;
using TableTennis4dView.Application.Queries.User;
...
namespace TableTennis4dView.API.Controllers
{
   ...
   //[Authorize(Roles = "Admin, Management")]
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("AssignUserPlayer")]
        [ProducesDefaultResponseType(typeof(int))]
        public async Task<ActionResult> AssignUserPlayer([FromBody]AssignUserPurchasedPlayerCommand command)
        {
            var result = await _mediator.Send(command);
            return Ok(result);
        }
